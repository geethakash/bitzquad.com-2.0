...
async function handleGET(req, res) {
    ...
    try {
        ...
        const count = await User.countDocuments({ ...query.search, deleted: false });
        const resp = await User.find({ ...query.search, deleted: false }, { ...query.select }, query.options);
...
async function handlePOST(req, res) {
    await mongodb.Connect();
    try {
        const resp1 = await User.create(req.body);
