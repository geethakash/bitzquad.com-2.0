...
const All = () => {
    ...
    const init = async () => {
        setLoading(true);
        // const writersres = await axios.get(`/api/news?${queryparser.Build({}, { writer: 1 })}`);
    ...
    const fetch = async () => {
        const response = await axios.get(`/api/news?${queryparser.Build({}, {}, selectedRows.value, selectedPage, selectedSortby.query)}`);
        if (response.data.status == 200) setNews(response.data.data);
    };
                                                ...
                                                <div className="w-8/12">
                                                    <div className="flex items-center justify-between px-4 pt-5 sm:px-6">
                                                        <div className="flex flex-1 justify-between sm:hidden">
                                                            ...
                                                            <a
                                                                onClick={() => {
                                                                    if (selectedPage < news.pages) setSelectedPage(selectedPage + 1);
                                                                }}
                                                        ...
                                                        <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                                                            <div>
                                                                <p className="text-sm text-gray-700">
                                                                    Showing <span className="font-medium">{selectedPage * news.limit + 1}</span> to{" "}
                                                                    <span className="font-medium">{selectedPage * news.limit + news.limit}</span> of{" "}
