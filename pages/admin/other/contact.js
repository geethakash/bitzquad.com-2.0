...
const All = () => {
    ...
    const init = async () => {
        setLoading(true);
        // const writersres = await axios.get(`/api/contact?${queryparser.Build({}, { writer: 1 })}`);
    ...
    const fetch = async () => {
        const response = await axios.get(`/api/contact?${queryparser.Build({}, {}, selectedRows.value, selectedPage, selectedSortby.query)}`);
        if (response.data.status == 200) setContact(response.data.data);
    };
                                                ...
                                                <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                                    <div className="flex items-center">
                                                        ...
                                                        <div className="ml-4">
                                                            ...
                                                            <div className="text-gray-500">{opt.createdAt}</div>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                    <div className="text-gray-900">{opt.category}</div>
                                                    <div className="text-gray-500">{opt.writer.name}</div>
                                                </td>
                                                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                    <span className="inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold leading-5 text-green-800">{opt.status}</span>
                                                </td>
                                                <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                                    <a href="#" className="text-indigo-600 hover:text-indigo-900">
                                                        Edit<span className="sr-only">, {opt.title}</span>
                                                    </a>
                                                </td>
                                            </tr>
                                        ))}
                                                ...
                                                <div className="w-8/12">
                                                    <div className="flex items-center justify-between px-4 pt-5 sm:px-6">
                                                        <div className="flex flex-1 justify-between sm:hidden">
                                                            ...
                                                            <a
                                                                onClick={() => {
                                                                    if (selectedPage < contact.pages) setSelectedPage(selectedPage + 1);
                                                                }}
                                                        ...
                                                        <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                                                            <div>
                                                                <p className="text-sm text-gray-700">
                                                                    Showing <span className="font-medium">{selectedPage * contact.limit + 1}</span> to{" "}
                                                                    <span className="font-medium">{selectedPage * contact.limit + contact.limit}</span> of{" "}
                                                                    <span className="font-medium">{contact.total}</span> results
                                                                </p>
                                                            </div>
                                                            <div>
