...
const Layout = ({ children }) => {
  ...
  for (let i = 0; i < navigation.length; i++) {
    if (router.asPath.includes(navigation[i].href))
      ...
      <div>
          ...
          <div className="flex min-h-0 flex-1 flex-col border-r border-gray-200 bg-white">
            <div className="flex flex-1 flex-col overflow-y-auto pt-5 pb-4">
              <div className="flex flex-shrink-0 items-center justify-center px-4">
                <img
                  className="h-8 w-auto"
                  src={sidebarCollapse ? Logo.src : LogoDark.src}
                  alt="Workflow"
                />
                {sidebarCollapse ? (
                  <ArrowRightIcon
              ...
              <nav
                className="mt-8 flex-1 space-y-1 bg-white px-2"
                aria-label="Sidebar"
              >
                {navigation.map((item) =>
                  !item.children ? (
                    <div key={item.name}>
                      <a
                        onClick={() => {
                          router.push(item.href || item.base, true);
                        }}
                        className={classNames(
                          item.current
                            ? "bg-gray-100 text-gray-900"
                    ...
                    <Disclosure as="div" key={item.name} className="space-y-1">
                      {({ open }) => (
                        <>
                          ...
                          >
                            <item.icon
                              className="mr-5 h-6 w-6 flex-shrink-0 text-gray-400 group-hover:text-gray-500"
                              aria-hidden="true"
                            />
        ...
        <div
          className={`flex flex-1 flex-col ${
            sidebarCollapse ? "md:pl-16" : "md:pl-64"
          }`}
        >
